<?php
class Ttk_Parser_Youku extends Ttk_Parser_Abstract
{
    ...
    public function _getYouku($vid)
	{
        $base = "http://v.youku.com/player/getPlaylist/VideoIDS/";
        $blink = $base.$vid;
        $link = $blink."/Pf/4/ctype/12/ev/1";
        $retval = Lamb_Http::quickGet($link, 5, false, $status);
        $bretval = Lamb_Http::quickGet($blink, 5, false, $status);
        if ($retval) {
